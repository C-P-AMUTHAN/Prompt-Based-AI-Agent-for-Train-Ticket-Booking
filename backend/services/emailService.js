...
const sendTicketEmail = async (ticket) => {
  try {
    ...
    console.log('[emailService] Preparing ticket email for:', ticket.email);
    console.log('[emailService] PDF buffer received:', ticket.pdfBuffer ? `${ticket.pdfBuffer.length} bytes` : 'null');
    console.log('[emailService] PDF buffer type:', ticket.pdfBuffer ? typeof ticket.pdfBuffer : 'not provided');
    console.log('[emailService] PDF buffer is Buffer:', ticket.pdfBuffer instanceof Buffer);

    const attachments = ticket.pdfBuffer && ticket.pdfBuffer.length > 0 ? [{
      filename: `ticket_${ticket.pnr}.pdf`,
      content: ticket.pdfBuffer
    }] : [];

    console.log('[emailService] Attachments to send:', attachments.length > 0 ? `1 PDF (${attachments[0].content.length} bytes)` : 'none');
    ...
    console.log('[emailService] Email send attempt:', { messageId: info && info.messageId });
    ...
    console.error('[emailService] Error sending email:', error && error.stack ? error.stack : error);
